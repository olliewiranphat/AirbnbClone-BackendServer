...
exports.addAccom = TryCatch(async (req, res) => {
  ...
  const createdAccomAmen = await prisma.accomAmen.createMany({
    data: amenity.map((item) => ({
      accommodationID: parseInt(accommodation.accommodationID),
      amenityID: parseInt(item.amenityID), // Using amenityID from frontend
      // quantity: parseInt(item.quantity), // Quantity of amenity
    })),
  });
  ...
  const findAmenities = await prisma.amenity.findMany({
    where: {
      amenityID: {
        in: amenity.map((item) => parseInt(item.amenityID)),
      },
    },
  });

  if (findAmenities.length !== amenity.length) {
    return createError(404, "No have Amenities yet!");
  }
